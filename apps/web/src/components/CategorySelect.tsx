  ...
  const options = useMemo(
    () => [
      ...
      ...categories
        ...
        .map((c) => ({ value: String(c.id), label: c.name, color: c.color ?? undefined })),
    ],
    [categories, t]
  );
      ...
      <SearchableSelect
        ...
        footer={
          <button
            type="button"
            className={cn(
              'flex w-full cursor-pointer items-center gap-2 border-t px-3 py-1.5 text-sm text-primary hover:bg-gray-50',
            )}
            onClick={() => {
              setBankOverride(null);
              setShowModal(true);
